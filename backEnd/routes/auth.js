...
let JWT_SECRET = "mirza@mirza"
// route-1 create a user using post "/api/auth/createuser" no login required
router.post('/createuser', [
    body('email', "Enter a valid email address").isEmail(),
    body('password', 'password must be atleast 5 cherector ').isLength({ min: 5 }),
    ...
    try {
        ...
        if (user) {
            success = false
            return res.status(400).json({success, error: "sorry a usr is already exist with same eamil" })
        }
